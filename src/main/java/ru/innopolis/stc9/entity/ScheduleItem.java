...
public class ScheduleItem {
    @Id
    @SequenceGenerator(name = "scheduleItemSeq", sequenceName = "scheduleItem_SEQUENCE", allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "scheduleItemSeq")
    private long id;
    @Column
    ...
    @ManyToMany
    @JoinTable(name = "groupsId",
            joinColumns = @JoinColumn(name = "group_Id"),
            inverseJoinColumns = @JoinColumn(name = ""))
    private Group groupItem;

    @ManyToMany
    @JoinTable(name = "teachSubjId",
            joinColumns = @JoinColumn(name = "group_Id"),
            inverseJoinColumns = @JoinColumn(name = ""))
